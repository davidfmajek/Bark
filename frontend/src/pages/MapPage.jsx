...
export function MapPage() {
  ...
  const locationCount = diningLocations.length;
  ...
  function focusLocationOnMap(location) {
    ...
    if (map) {
      map.flyTo(location.position, Math.max(map.getZoom(), 18), {
        duration: 0.8,
      });
    }
  ...
  return (
    <div className={`relative min-h-[calc(100vh-3.5rem)] overflow-hidden ${dark ? 'bg-[#0f1219]' : 'bg-white'}`}>
      ...
      <section className="relative z-10 mx-auto flex min-h-[calc(100vh-3.5rem)] w-full max-w-7xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        <header className={`flex flex-wrap items-end justify-between gap-3 ${dark ? 'text-white' : 'text-black'}`}>
          <div>
            <h1 className="font-display text-3xl font-black tracking-tight">Campus Dining Map</h1>
            <p className={`mt-1 text-sm ${dark ? 'text-white/70' : 'text-black/60'}`}>
             Explore all of the dining locations on campus.
            </p>
          </div>
          <div className={`rounded-2xl border px-4 py-2 text-sm font-medium ${dark ? 'border-white/15 bg-white/[0.04] text-white/80' : 'border-black/10 bg-[#faf7ef] text-black/70'}`}>
            {loading ? 'Loading locations...' : `${locationCount} active locations`}
          </div>
        </header>
        ...
        <div className={`grid flex-1 gap-4 lg:grid-cols-[minmax(0,1fr)_320px] ${dark ? 'text-white' : 'text-black'}`}>
          ...
          <aside className={`flex max-h-[85vh] min-h-[460px] flex-col rounded-[2rem] border p-5 backdrop-blur ${dark ? 'border-white/10 bg-white/[0.05]' : 'border-black/10 bg-white/90'}`}>
            ...
            <div className={`mt-5 flex-1 space-y-3 overflow-y-auto pr-1 text-sm ${dark ? 'text-white/80' : 'text-black/75'}`}>
              {loading ? (
                <div className={`rounded-2xl border p-4 text-sm ${dark ? 'border-white/10 bg-black/10 text-white/75' : 'border-black/10 bg-[#faf7ef] text-black/70'}`}>
                  Loading locations from the directory...
                </div>
              ) : locationCount === 0 ? (
                <div className={`rounded-2xl border p-4 text-sm ${dark ? 'border-white/10 bg-black/10 text-white/75' : 'border-black/10 bg-[#faf7ef] text-black/70'}`}>
                  No active dining locations are available right now.
                </div>
              ) : (
                diningLocations.map((location) => (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    onClick={() => focusLocationOnMap(location)}
                    onKeyDown={(event) => {
                    ...
                    className={`cursor-pointer rounded-2xl border p-4 transition-colors ${
                      dark
                        ? activeLocationId === location.id
                          ? 'border-[#f5bf3e]/70 bg-[#f5bf3e]/10'
                          : 'border-white/10 bg-black/10 hover:border-[#f5bf3e]/40'
                        : activeLocationId === location.id
                          ? 'border-[#b18933] bg-[#f4e7c7]'
                          : 'border-black/10 bg-[#faf7ef] hover:border-[#d4b46f]'
                    }`}
                  >
