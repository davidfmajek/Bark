  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm">
      <div className={`w-full max-w-sm rounded-2xl border p-6 shadow-2xl ${dark ? 'border-white/10 bg-[#161b26]' : 'border-black/10 bg-white'}`}>
        <p className={`break-words text-sm leading-relaxed [overflow-wrap:anywhere] ${dark ? 'text-white/80' : 'text-black/70'}`}>{message}</p>
      ...
      >
        <div className="mb-4 flex shrink-0 items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <h3 className={`break-words text-lg font-semibold [overflow-wrap:anywhere] ${dark ? 'text-white' : 'text-black'}`}>{title}</h3>
            {subtitle ? <p className={`mt-1 break-words text-sm ${dark ? 'text-white/60' : 'text-black/60'}`}>{subtitle}</p> : null}
          </div>
          <button onClick={onClose} className={`shrink-0 rounded-lg p-1.5 transition-colors ${dark ? 'hover:bg-white/10' : 'hover:bg-black/5'}`}>
            <X className="h-4 w-4" />
          </button>
        </div>
